...
if (typeof _jammer == "Land_DataTerminal_01_F") then //check if the object Zeus placed is a Data Terminal aka Jammer
	{
		...
		{[[_x]] execVM "Scripts\RadioJammer\TFARjamRadios.sqf";} forEach (allMissionObjects "Land_DataTerminal_01_F");//activate Rebel's radio Jamming script, ALL CREDIT TO HIM FOR THIS, I MADE 0 EDITS TO HIS WORK
		
		_jamPos = getPos _jammer;//get the position of the jammer
		_jamSource = "Land_HelipadEmpty_F" createVehicle _jamPos;//create an invisible helipad to use as our sound source
		...
		_jammer addAction ["Shutdown Jammer","Scripts\RadioJammer\jammer_shutdown.sqf",[_jammer,_jamSource],1,false,true,"","_this distance _target < 3"];//add the action to shut it down
