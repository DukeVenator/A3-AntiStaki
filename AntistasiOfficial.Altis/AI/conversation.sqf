...
call {
	if (typeOf _unit == (CIV_specialUnits select 2)) exitWith {
		...
		line2 = ["Stranger", "Return in civilian atire, then we'll talk."];

	};
	if (typeOf _unit == (CIV_specialUnits select 1)) exitWith {
		line1 = ["Nomad", "You have some nerves showing up like this! You stingy bastard now donate money to duke he is poor man."];
